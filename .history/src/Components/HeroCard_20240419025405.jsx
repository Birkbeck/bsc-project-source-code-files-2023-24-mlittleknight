...
const HeroCard = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/english-phrases/")
      .then((response) => response.json())
      .then((data) => {
        const randomIndex = Math.floor(Math.random() * data.length);
        const randomVerb = data[randomIndex].phrase;

        // Extract description from the selected verb
        const description = randomVerb.split(" ")[0].trim();

        // Fetch Definition for the selected verb from dictionaryapi.dev
        fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${description}`)
          .then((response) => response.json())
          .then((definitionData) => {
            const description =
